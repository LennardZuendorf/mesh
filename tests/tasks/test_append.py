...
discoverable through the inbound lens (``mesh graph <id> --direction in``,
...
from mesh.core.context import graph_query
from mesh.core.tasks import TaskNotFoundError, append_task, finish_task
...
def _seed_note(vault: Path, note_id: str, title: str = "Note") -> Path:
    """Write a minimal mesh note straight to disk under notes/."""
    meta: dict[str, object] = {
        "id": note_id,
        "type": "note",
        "title": title,
...
def test_cli_append_owner_flag_stamps_the_acting_agent(mesh_config: Path, vault: Path) -> None:
    ...
    path = _seed_task(vault)
    result = _invoke(["--owner", "bob", "task", "append", "t-seed", "logged", "--timestamp"])
    assert result.exit_code == 0, result.output
    content = _reload(path).content
    stamp_line = next(line for line in content.splitlines() if _ISO_UTC.search(line))
...
def test_cli_append_json_object(mesh_config: Path, vault: Path) -> None:
