...
def no_agent_config(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, vault: Path) -> Path:
    """A config with no ``[core].agent`` and no ``$MESH_AGENT`` override."""
...
def test_mesh_note_list_body_returns_matching_on_disk_notes(cfg: Config, vault: Path) -> None:
    create_note(cfg, "First Listed Note", body="one")
    n2 = create_note(cfg, "Second Listed Note", body="two")

    entries = asyncio.run(server.app.call_tool("mesh_note_list", {"limit": 1}))
...
def test_mesh_task_get_body_returns_on_disk_frontmatter(cfg: Config, vault: Path) -> None:
    task = create_task(cfg, "Gettable Task", body="task body text")

    reply = _call("mesh_task_get", {"id": task.id})
...
def test_mesh_note_new_body_writes_note_to_disk(cfg: Config, vault: Path) -> None:
    reply = _call("mesh_note_new", {"title": "MCP-Created Note", "body": "mcp body"})
    note_id = reply["id"]
...
def test_mesh_note_append_body_appends_on_disk(cfg: Config, vault: Path) -> None:
    note = create_note(cfg, "Appendable Note", body="original")
    reply = _call("mesh_note_append", {"target": note.id, "text": "added-via-mcp"})
...
def test_mesh_task_new_body_writes_task_to_disk(cfg: Config, vault: Path) -> None:
    reply = _call("mesh_task_new", {"title": "MCP-Created Task", "body": "mcp task body"})
    task_id = reply["id"]
...
def test_mesh_task_append_body_appends_on_disk(cfg: Config, vault: Path) -> None:
    task = create_task(cfg, "Appendable Task", body="original task body")
    reply = _call("mesh_task_append", {"task_id": task.id, "text": "added-via-mcp"})
    assert reply["id"] == task.id
...
def test_mesh_note_update_body_moves_note_and_bumps_tags(cfg: Config, vault: Path) -> None:
    note = create_note(cfg, "Updatable Note", body="x", note_type="note")
    _call("mesh_note_update", {"target": note.id, "tags": "+urgent", "new_type": "decision"})
...
def test_mesh_task_claim_body_writes_claimed_by_on_disk(cfg: Config, vault: Path) -> None:
    task = create_task(cfg, "Claimable Task", body="x")
    reply = _call("mesh_task_claim", {"task_id": task.id, "claimer": "mcp-agent"})
...
def test_mesh_task_claim_no_agent_identity_raises_validation_error(
    no_agent_config: Path, vault: Path
) -> None:
    """locks.py-adjacent guard in server.py: no claimer and no configured agent."""
    from mesh.core.tasks import create_task as _create_task

    task = _create_task(load_config(), "Claim Needs Identity", body="x")

    with pytest.raises(ToolError) as exc_info:
        asyncio.run(server.app.call_tool("mesh_task_claim", {"task_id": task.id}))
...
def test_mesh_task_release_body_clears_claimed_by_on_disk(cfg: Config, vault: Path) -> None:
    from mesh.core.tasks import claim_task

    task = create_task(cfg, "Releasable Task", body="x")
    claim_task(cfg, task.id, "mcp-agent")

    reply = _call("mesh_task_release", {"task_id": task.id, "owner": "mcp-agent"})
...
def test_mesh_task_release_no_agent_identity_raises_validation_error(
    no_agent_config: Path, vault: Path
) -> None:
    from mesh.core.tasks import claim_task as _claim_task
    from mesh.core.tasks import create_task as _create_task
    ...
    with pytest.raises(ToolError) as exc_info:
        asyncio.run(server.app.call_tool("mesh_task_release", {"task_id": task.id}))
    payload = json.loads(str(exc_info.value))
    assert payload["kind"] == "validation"
    assert "agent identity" in payload["message"]
...
def test_mesh_task_finish_body_moves_task_to_done_on_disk(cfg: Config, vault: Path) -> None:
    task = create_task(cfg, "Finishable Task", body="x")
    reply = _call("mesh_task_finish", {"task_id": task.id, "outcome": "shipped via mcp"})
...
def test_mesh_task_update_body_writes_new_fields_on_disk(cfg: Config, vault: Path) -> None:
    task = create_task(cfg, "Updatable Task", body="x", priority="low")
    _call("mesh_task_update", {"task_id": task.id, "priority": "high", "title": "Renamed via MCP"})
...
def test_mesh_task_cancel_body_moves_task_to_done_cancelled_on_disk(
    cfg: Config, vault: Path
) -> None:
    task = create_task(cfg, "Cancellable Task", body="x")
    reply = _call("mesh_task_cancel", {"task_id": task.id, "reason": "no longer needed"})
...
# --------------------------------------------------------------------------- #
# The generic-MeshError fallback kind (server.py:1181) — no live subclass    #
# reaches it today; unit-tested directly against a throwaway subclass         #
# --------------------------------------------------------------------------- #


def test_error_kind_falls_back_to_code_table_for_an_unlisted_mesh_error() -> None:
    """Every concrete ``MeshError`` subclass in the codebase is already named
    ...
    class _FutureError(MeshError):
        code = 3

    assert server._error_kind(_FutureError("x")) == "not_found"

    class _UnknownCodeError(MeshError):
        code = 99
