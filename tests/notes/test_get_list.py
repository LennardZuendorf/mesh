...
Exercises R3 (Read / list): :func:`mesh.core.notes.get_note` and
:func:`mesh.core.notes.list_notes` plus the ``mesh note get`` / ``mesh note
list`` CLI surface. ``get`` yields frontmatter + a 200-char body preview
(``--full`` / ``--meta-only`` / ``--related`` switch the shape); ``list`` only
surfaces files carrying a valid mesh ``n-`` id (foreign files are skipped) and
...
def _seed_foreign(vault: Path, name: str, meta: dict[str, object] | None = None) -> Path:
    """Write a non-mesh Markdown file (no valid ``n-`` id) under ``notes/``."""
...
# --------------------------------------------------------------------------- #
# list_notes (core) — mesh-id gate + filters + sort/limit                     #
# --------------------------------------------------------------------------- #


def test_list_skips_files_without_mesh_id(cfg: Config, vault: Path) -> None:
...
def test_cli_get_default_preview_truncates_at_200(mesh_config: Path, vault: Path) -> None:
    body = "A" * 250
...
def test_cli_get_full_shows_whole_body(mesh_config: Path, vault: Path) -> None:
    body = "A" * 250
...
def test_cli_get_meta_only_omits_body(mesh_config: Path, vault: Path) -> None:
...
def test_cli_get_related_only(mesh_config: Path, vault: Path) -> None:
    _seed_note(
        vault,
        note_id="n-seed",
...
def test_cli_get_not_found_exits_3(mesh_config: Path, vault: Path) -> None:
    _seed_note(vault, note_id="n-seed")
    result = _invoke(["note", "get", "n-missing"])
    assert result.exit_code == 3


def test_cli_get_foreign_file_exits_3(mesh_config: Path, vault: Path) -> None:
...
def test_cli_get_broken_mesh_file_exits_3(mesh_config: Path, vault: Path) -> None:
    """Finding #3: a mesh-id file with malformed frontmatter maps to exit 3, not a traceback."""
...
def test_cli_get_ambiguous_slug_exits_2_lists_ids(mesh_config: Path, vault: Path) -> None:
...
def test_cli_get_json_single_object(mesh_config: Path, vault: Path) -> None:
...
def test_cli_get_json_full_body(mesh_config: Path, vault: Path) -> None:
    body = "A" * 250
    _seed_note(vault, note_id="n-seed", body=body)
...
def test_cli_get_json_meta_only_has_no_body(mesh_config: Path, vault: Path) -> None:
    _seed_note(vault, note_id="n-seed", body="x")
    obj = json.loads(_invoke(["--json", "note", "get", "n-seed", "--meta-only"]).output)
    assert "body" not in obj


def test_cli_get_json_related_is_object(mesh_config: Path, vault: Path) -> None:
...
def test_cli_get_quiet_emits_id_only(mesh_config: Path, vault: Path) -> None:
    _seed_note(vault, note_id="n-seed", title="Seed", body="body")
...
def test_cli_list_surfaces_mesh_notes_only(mesh_config: Path, vault: Path) -> None:
    _seed_note(vault, note_id="n-real", title="Real")
...
def test_cli_list_json_is_array(mesh_config: Path, vault: Path) -> None:
...
def test_cli_list_quiet_one_id_per_line(mesh_config: Path, vault: Path) -> None:
    _seed_note(vault, note_id="n-a", updated=_now())
    _seed_note(vault, note_id="n-b", updated=_now() - timedelta(minutes=1))
...
def test_cli_list_tags_and_filter(mesh_config: Path, vault: Path) -> None:
    _seed_note(vault, note_id="n-both", tags=["ndc", "flights"], updated=_now())
    _seed_note(
        vault,
        note_id="n-one",
...
def test_cli_list_any_tag_filter(mesh_config: Path, vault: Path) -> None:
    _seed_note(vault, note_id="n-both", tags=["ndc", "flights"], updated=_now())
    _seed_note(
        vault,
        note_id="n-one",
...
def test_cli_list_owner_filter(mesh_config: Path, vault: Path) -> None:
    _seed_note(vault, note_id="n-a", owner="alice", updated=_now())
...
def test_cli_list_type_filter(mesh_config: Path, vault: Path) -> None:
    _seed_note(vault, note_id="n-note", note_type="note", updated=_now())
    _seed_note(
        vault,
        note_id="n-dec",
...
def test_cli_list_since_filter(mesh_config: Path, vault: Path) -> None:
    _seed_note(vault, note_id="n-recent", updated=_now() - timedelta(days=1))
    _seed_note(vault, note_id="n-old", updated=_now() - timedelta(days=30))
...
def test_cli_list_sort_title(mesh_config: Path, vault: Path) -> None:
    _seed_note(vault, note_id="n-b", title="Bravo", updated=_now())
    _seed_note(vault, note_id="n-a", title="Alpha", updated=_now() - timedelta(minutes=1))
...
def test_cli_list_limit(mesh_config: Path, vault: Path) -> None:
    for i in range(5):
        _seed_note(vault, note_id=f"n-{i:02d}", updated=_now() - timedelta(minutes=i))
...
def test_cli_list_invalid_sort_exits_2(mesh_config: Path, vault: Path) -> None:
    _seed_note(vault, note_id="n-a")
...
def test_get_note_by_slug_skips_malformed_yaml(cfg: Config, vault: Path) -> None:
    """The slug scan reads every mesh file; a malformed one must not crash it."""
    _seed_note(vault, note_id="n-good", title="Good One")
    _seed_malformed(vault, "n-broken")  # mesh-id stem → enters the slug scan
    assert get_note(cfg, "good-one").note.id == "n-good"


def test_get_note_malformed_mesh_file_is_not_found(cfg: Config, vault: Path) -> None:
    """A mesh file with corrupt YAML resolves to not-found, mirroring get_task."""
    _seed_malformed(vault, "n-broken")
    with pytest.raises(NoteNotFoundError):
