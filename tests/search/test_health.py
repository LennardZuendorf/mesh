...
explicit in one JSON payload, both via :func:`mesh.core.search.search_health`
directly and through the ``mesh search --health`` CLI flag — and it must
degrade cleanly (never raise) even when ``indexed`` is entirely absent.

agent-usability/4 adds the MCP mirror, ``mesh_health`` (``mesh/mcp/server.py``)
...
``mesh search --health`` CLI invocation over the same mocked gate state —
...
from mesh.core.search import search_health
...
def _write_config(
    *,
    ...
    """Write a real ``config.toml`` and export ``MESH_CONFIG_PATH`` — needed
...
def test_cli_search_health_ignores_query_and_never_performs_a_search(
    ...
    monkeypatch.setattr("mesh.cli.search.query_search", _boom)
    monkeypatch.setattr("mesh.core.search._daemon_up", lambda: True)
    monkeypatch.setattr(indexed_client, "indexed_available", lambda: True)
...
# --------------------------------------------------------------------------- #
# MCP: mesh_health (agent-usability/4)                                      #
# --------------------------------------------------------------------------- #


def _mcp_health() -> dict[str, Any]:
    """Dispatch the real *registered* ``mesh_health`` tool (not the bare
    module function), mirroring how an MCP client actually calls it."""
    dispatched = asyncio.run(server.app.call_tool("mesh_health", {}))
...
def test_mcp_health_mirrors_core_search_health_gates(
    ...
    """``mesh_health`` reports the same ``mode``/gates ``search_health``
    ...
    monkeypatch.setattr("mesh.core.search._daemon_up", lambda: gate_mocks["daemon_up"])
    monkeypatch.setattr(
...
def test_mcp_health_matches_cli_health_byte_for_byte(
    mesh_config: Path, monkeypatch: pytest.MonkeyPatch, gate_mocks: dict[str, bool]
) -> None:
    """The load-bearing anti-drift assertion: over the *identical* mocked gate
    state, the registered MCP tool and the real ``mesh search --health`` CLI
...
def test_mcp_health_is_a_pure_delegate_not_a_reimplementation(
    ...
    exactly what it returned — ``mesh_health`` has no gate logic of its own
