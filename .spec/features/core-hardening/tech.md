...
| 14 | `indexed_client.register_hook`, `reindex` alias | **Reject — delta from brief** | Re-measured: `register_hook` genuinely has zero *production* callers (the daemon inlines its own lambda at `server.py`, confirming the original claim), but it is not dead in the sense "safe to delete" — `tests/search/test_freshness.py::test_register_hook_is_explicit_and_config_bound` and `tests/search/test_indexed_client.py::test_register_hook_drives_incremental_update` call it directly and assert its behaviour. `reindex` turned out to be mischaracterised as dead code: it has exactly one production caller (`admin.py::reindex_command`) and its own dedicated test (`test_reindex_delegates_to_full_rebuild`) — it is a one-line *named delegate*, not duplication. Deleting either would require deleting whole test functions, not just adjusting import paths — the unit's own binding constraint ("zero test edits beyond import paths"). Left in place; recorded here rather than actioned silently. |
