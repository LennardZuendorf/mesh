"""FastMCP server — the agent-facing ``brain_*`` tool surface (memory/1).
...
Unlike the CLI, MCP parameters are *typed fields*, not flag strings: ``brain_note_get``
...
from brain.cli.search import _hit_dict, _query_search
from brain.core.activity import recent_activity
from brain.core.context import build_context
...
def brain_note_get(id: str) -> dict[str, Any]:
...
def brain_note_list(
    ...
    """List brain notes with tag/owner/type/recency filters, sorted and capped."""
    config = load_config()
    views = list_notes(
        config,
        tags=tags,
...
def brain_task_get(id: str) -> dict[str, Any]:
...
def brain_task_list(
    ...
    """List brain tasks (open and done) with status/owner/mine filters, sorted."""
    config = load_config()
    views = list_tasks(
        config,
        status=status,
...
def brain_build_context(seed_id: str, depth: int = 1) -> list[dict[str, Any]]:
...
def brain_task_claim(task_id: str, claimer: str | None = None) -> dict[str, Any]:
...
def brain_task_finish(task_id: str, outcome: str | None = None) -> dict[str, Any]:
...
def brain_task_cancel(task_id: str, reason: str | None = None) -> dict[str, Any]:
...
def _register() -> None:
    """Register every ``brain_*`` tool with its annotation; called once at import."""
    # Read-only.
    app.tool(brain_note_get, annotations=_READ_ONLY)
    app.tool(brain_note_list, annotations=_READ_ONLY)
    app.tool(brain_task_get, annotations=_READ_ONLY)
    app.tool(brain_task_list, annotations=_READ_ONLY)
    app.tool(brain_search, annotations=_READ_ONLY)
    app.tool(brain_recent_activity, annotations=_READ_ONLY)
    app.tool(brain_build_context, annotations=_READ_ONLY)
    # Write (no special hint).
    app.tool(brain_note_new)
    app.tool(brain_note_append)
    app.tool(brain_task_new)
    # Idempotent.
    app.tool(brain_note_update, annotations=_IDEMPOTENT)
    app.tool(brain_task_claim, annotations=_IDEMPOTENT)
    app.tool(brain_task_finish, annotations=_IDEMPOTENT)
    app.tool(brain_task_update, annotations=_IDEMPOTENT)
    # Destructive.
    app.tool(brain_task_cancel, annotations=_DESTRUCTIVE)
...
def main() -> None:  # pragma: no cover - process entry point
    """Serve the brain MCP tools (stdio transport). Wired as ``python -m brain.mcp.server``."""
    app.run()
