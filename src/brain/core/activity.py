...
shared by the ``brain recent-activity`` CLI command and (later) the
``brain_recent_activity`` MCP tool. It is a *lens*, not a store: the rows come
...
:meth:`brain.daemon.client.DaemonClient.activity_recent`, whose own fallback
contract already routes to :func:`brain.index.watch.scan_recent` on a socket-down
