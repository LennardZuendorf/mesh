...
primitive, shared by the ``brain build-context`` CLI command and (later) the
``brain_build_context`` MCP tool. Starting at a seed id it walks the ``related``
...
read straight off disk via :func:`brain.core.notes.get_note` (``n-`` ids) and
:func:`brain.core.tasks.get_task` (``t-`` ids), so it behaves identically with
...
from brain.core.notes import NoteError, get_note
from brain.core.tasks import TaskError, get_task
...
def _resolve_entry(config: Config, entry_id: str) -> dict[str, Any] | None:
    ...
    :func:`brain.core.tasks.get_task`; anything else (``n-`` ids, and — matching
    ``note`` CLI behaviour — a title slug) through
    :func:`brain.core.notes.get_note`. Not-found, ambiguous, or malformed nodes
