...
Brain owns the *interface* to search, not the ranking: this module shells the
...
* **:func:`search`** — run ``indexed index search "<q>" --collection <c> --json
  ...
  (non-brain) files surface with ``id=None``; paths that escape the vault sandbox
  ...
  :func:`~brain.index.fallback.search_fallback`.
...
* **:func:`incremental_update` / :func:`full_rebuild` / :func:`reindex`** — keep the
  ...
  thread and let failures propagate. ``reindex`` is the delegate ``brain reindex``
  calls.

* **:func:`register_hook`** — subscribe :func:`incremental_update` to the daemon's
  watcher change-hook registry (:func:`brain.index.watch.register_change_hook`) so a
...
from brain.index.tagpull import (
    matches_filters,
    meta_tags,
    read_row,
    to_result,
...
from brain.index.watch import register_change_hook
...
def search(
    ...
    delegates to :func:`~brain.index.fallback.search_fallback` (forwarding ``quiet``
...
def incremental_update(config: Config, path: Path) -> None:
    ...
    whole lifetime. Incremental freshness is best-effort; ``brain reindex`` (a full
...
def reindex(config: Config) -> None:
    """Full rebuild — the delegate ``brain reindex`` calls."""
    full_rebuild(config)
