"""Admin surface: ``daemon`` lifecycle, ``brain status``, ``brain reindex``.
...
daemon's founding rule: it is an **accelerator, never a gate**. ``brain status``
...
* **``daemon start|stop|status``** — supervise a detached socket server. Its PID
  lives in a state file beside the socket (``$XDG_RUNTIME_DIR/brain.pid``, else
  ``~/.brain/run/brain.pid``), written atomically (temp file + ``os.replace``).
...
* **``brain status``** — note count, tasks-by-status, freshness (newest vault
  mtime + age), dangling wikilinks (via :func:`brain.core.wikilinks.find_dangling`),
  and stale ``O_EXCL`` locks (PID dead **or** age > 300 s, reusing the canonical
  rule from :mod:`brain.storage.locks`). Strictly read-only — it never bumps
  ``updated`` nor rewrites a file.
* **``brain reindex``** — delegates to the search feature's ``indexed`` client.
...
from brain.core.notes import list_notes
from brain.core.tasks import list_tasks
from brain.core.wikilinks import find_dangling
...
from brain.storage.files import atomic_write
...
from brain.storage.locks import _is_stale, _pid_alive
...
def spawn_daemon() -> int:
    ...
    Runs :func:`brain.daemon.server.serve` in a fresh, session-leading process; the
    child inherits ``$XDG_RUNTIME_DIR`` / ``$BRAIN_CONFIG_PATH`` so it resolves the
    ...
    proc = subprocess.Popen(  # noqa: S603 - fixed argv, no shell, no user input
        [sys.executable, "-c", "from brain.daemon.server import serve; serve()"],
...
def scan_stale_locks(config: Config) -> list[Path]:
    ...
    exact rule enforced on acquire (:func:`brain.storage.locks._is_stale`), reused
    here so ``brain status`` and the locker never disagree.
    """
...
def vault_status(config: Config) -> dict[str, Any]:
    ...
    freshness (newest brain-file mtime + its age in seconds), dangling wikilink
...
def _status_lines(report: dict[str, Any]) -> list[str]:
    """Render the human-readable ``brain status`` block."""
    tasks = report["tasks"]
