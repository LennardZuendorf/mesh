"""Admin surface: ``mesh init``, ``daemon`` lifecycle, ``status``, ``reindex``.
...
daemon's founding rule: it is an **accelerator, never a gate**. ``mesh status``
...
* **``mesh init``** (agent-usability/7) — writes ``~/.mesh/config.toml`` (or
  ``$MESH_CONFIG_PATH``): vault path, agent identity, roster, search settings.
...
* **``daemon start|stop|status``** — supervise a detached socket server. Its PID
  lives in a state file beside the socket (``$XDG_RUNTIME_DIR/mesh.pid``, else
  ``~/.mesh/run/mesh.pid``), written atomically (temp file + ``os.replace``).
...
* **``mesh status``** — note count, tasks-by-status, freshness (newest vault
  mtime + age), dangling wikilinks (via :func:`mesh.core.wikilinks.find_dangling`),
  and stale ``O_EXCL`` locks (PID dead **or** age > 300 s, reusing the canonical
  rule from :mod:`mesh.storage.locks`). Served from the daemon's warm index when
  it is up and from a direct scan when it is down — same payload either way, via
  :meth:`mesh.daemon.client.DaemonClient.vault_status`. Strictly read-only — it
  never bumps ``updated`` nor rewrites a file.
* **``mesh reindex``** — delegates to the search feature's ``indexed`` client
  (:func:`mesh.index.indexed_client.reindex`). A missing binary or a non-zero
...
from mesh.core.lenses import TASK_STATUSES
...
from mesh.schemas.config import load_config, resolve_config_path
from mesh.storage.files import atomic_write

# Reuse the canonical liveness rule rather than re-deriving it.
from mesh.storage.locks import _pid_alive

_PID_NAME = "mesh.pid"
# ``mesh init`` defaults — a first run with no flags at all still produces a
# working config (agent-usability/7's load-bearing test: note new / task list
# actually succeed afterward, not just "a file exists").
_DEFAULT_VAULT_PATH = Path.home() / ".mesh" / "vault"
_DEFAULT_AGENT = "agent"
# The staleness window for the per-agent "stale claims" count in ``mesh
...
def spawn_daemon() -> int:
    ...
    Runs :func:`mesh.daemon.server.serve` in a fresh, session-leading process; the
    child inherits ``$XDG_RUNTIME_DIR`` / ``$MESH_CONFIG_PATH`` so it resolves the
    ...
    proc = subprocess.Popen(  # noqa: S603 - fixed argv, no shell, no user input
        [sys.executable, "-c", "from mesh.daemon.server import serve; serve()"],
...
def _emit(ctx: typer.Context, payload: dict[str, Any], human: str) -> None:
    ...
    :func:`mesh.cli._output.emit_mutation`'s note/task envelope, so this stays
...
def render_config_toml(
    *,
    ...
    (:meth:`~mesh.schemas.config.SearchConfig.threshold_explicit`), so
...
def init_command(
    ...
    """Write ~/.mesh/config.toml (or $MESH_CONFIG_PATH); refuses to overwrite without --force.
    ...
    with cli_errors():
        ...
        resolved_agent = agent or os.environ.get("MESH_AGENT") or _DEFAULT_AGENT
...
def _agent_breakdown(views: list[TaskView]) -> dict[str, dict[str, int]]:
    ...
    the task list ``mesh status`` already fetches (:meth:`DaemonClient.task_list`
...
def status_command(ctx: typer.Context) -> None:
    ...
    with cli_errors():
        ...
        # ``mesh init``'s documented behaviour.
...
def _status_lines(report: dict[str, Any]) -> list[str]:
    """Render the human-readable ``mesh status`` block."""
    tasks = report["tasks"]
