"""``mesh search`` command surface — the recall verb.

``search`` is one leaf command (not a sub-verb group), so it is a Typer whose
callback *is* the command: ``mesh search "<q>"`` and ``mesh search --tags x``
both land here. Routing (product R2/R4):

* no query, ``--tags`` (or nothing) → the frontmatter tag pull via
  :meth:`~mesh.daemon.client.DaemonClient.tag_pull` (``score=1.0``, meta-only) —
  served from the daemon's warm index when it is up, from the identical corpus
  walk (:func:`~mesh.index.tagpull.tagpull`) when it is down;
* a query → hybrid recall via :func:`~mesh.index.indexed_client.search` **when**
  ...
  :func:`~mesh.index.fallback.search_fallback`, which emits its own single stderr
  degradation notice.
...
:func:`~mesh.core.search.search_health`) whether hybrid ``indexed`` is
...
:func:`mesh.cli._output.coalesce_flags`. ``--json`` is accepted for parity
...
:func:`~mesh.core.search.query_search` already took to suppress the
...
def search_command(
    ...
    with cli_errors():
        if query is None:
            ...
            # marker on ``mesh_search`` hits is MCP's replacement for having
