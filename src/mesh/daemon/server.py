...
owns the *transport* — a unix socket at ``$XDG_RUNTIME_DIR/mesh-<digest>.sock``
...
:mod:`mesh.daemon.client`), so every write works with the daemon down.
...
a :class:`~mesh.index.warm.VaultIndex` (via a
:class:`~mesh.index.watcher.Watcher`) *before* the socket accepts a single
...
:func:`~mesh.index.indexed_client.incremental_update` on the server-owned
:class:`~mesh.index.watcher.ChangeHooks` registry, so every vault edit
...
from mesh.core.lenses import status_inputs
from mesh.core.notes import MetaRow, NoteFilter, in_note_scope, select_notes
from mesh.core.tasks import TaskFilter, in_task_scope, select_tasks
from mesh.daemon.client import default_socket_path, entity_row, vault_id
from mesh.index.indexed_client import incremental_update
from mesh.index.tagpull import TagPullFilter, select_tagpull
from mesh.index.warm import DEFAULT_RECENT_LIMIT, IndexEntry, VaultIndex
...
class DaemonServer:
    ...
    def _warm_handlers(self) -> dict[str, Handler]:
        ...
        def vault_status(_params: dict[str, Any]) -> dict[str, Any]:
            ...
            # ``status_report``. See :func:`mesh.core.lenses.status_inputs`.
