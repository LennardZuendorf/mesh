"""FastMCP server — the agent-facing ``mesh_*`` tool surface (memory/1).
...
Unlike the CLI, MCP parameters are *typed fields*, not flag strings: ``mesh_note_get``
...
* **read-only** (``readOnlyHint``) — ``mesh_note_get`` / ``mesh_note_list`` /
  ``mesh_task_get`` / ``mesh_task_list`` / ``mesh_search`` /
  ``mesh_recent_activity`` / ``mesh_build_context`` / ``mesh_graph`` /
  ``mesh_project`` / ``mesh_session_start`` (the warm-start composite —
  team-awareness/10 — reads three lenses and writes nothing) /
  ``mesh_health`` (agent-usability/4 — recall-path reachability, not vault
  contents or admin state; see below for why this one is exposed and
  ``status`` is not);
* **idempotent** (``idempotentHint``) — ``mesh_note_update`` / ``mesh_task_claim`` /
  ``mesh_task_finish`` / ``mesh_task_update`` / ``mesh_task_release``
  (re-running lands the same state — a release re-applied to an already-open
  task is a no-op, mirroring ``mesh_task_claim``'s same-agent no-op);
* **write** (no special hint) — ``mesh_note_new`` / ``mesh_note_append`` /
  ``mesh_task_new`` / ``mesh_task_append`` (each call is a genuine mutation,
  not idempotent replay — appending the same text twice appends it twice);
* **destructive** (``destructiveHint``) — ``mesh_task_cancel`` (a one-way lifecycle move).
...
``mesh_task_release`` *does* ship here (team-awareness/10 — release is a shipped
...
``mesh_health`` (agent-usability/4) is exposed despite the withheld-admin rule
...
reachability signal the CLI already exposes via ``mesh search --health``. It
carries no vault contents, no daemon control verb, and no way to mutate
anything — a pure read of :func:`~mesh.core.search.search_health`'s four
static gates, used *without* running a query. ``mesh_search`` results carry
...
non-degrading distinction). Unlike ``mesh_health``'s gate prediction, the
per-hit ``mode`` is *observed* — :func:`~mesh.core.search.query_search`
...
``mesh_health`` call just to know whether to trust it, and gets a stronger
guarantee than that second call could give anyway. The marker is MCP-only:
the CLI's ``hit_dict`` shape (:func:`mesh.core.search.hit_dict`) stays
unchanged for existing scripts, and a CLI caller already has ``mesh search
...
import time by :func:`mesh.mcp.instructions.build_instructions` from a guarded
...
read a local skill directory; see ``mesh/mcp/instructions.py`` for its content
...
from mesh.core.search import hit_dict, query_search, resolve_effective_threshold, search_health
...
from mesh.storage.locks import LockError


def _startup_config() -> Config | None:
    """Guarded config load for the ``instructions`` block (agent-usability/1).

    A missing ``config.toml`` raises :class:`~mesh.schemas.config.ConfigMissingError`
    ...
    The individual ``mesh_*`` tools *do* still fail per the normal error
    contract once actually called: ``ConfigMissingError`` is a
    :class:`~mesh.core.errors.MeshError`, a plain ``Exception``, so every
    ...
    ``mesh init`` — proven by driving real registered tools with a missing
...
app = FastMCP("mesh", instructions=build_instructions(_startup_config()))
...
def mesh_note_list(
    ...
    """List mesh notes with tag/owner/type/recency filters, sorted and capped."""
    config = load_config()
...
def mesh_task_list(
    ...
    """List mesh tasks (open and done) with status/owner/mine/project filters, sorted.
...
def mesh_search(
    ...
    the branch :func:`~mesh.core.search.query_search` *actually took* for
    this call, not a prediction from :func:`~mesh.core.search.search_health`'s
    ...
    independently. Call ``mesh_health`` for the standalone reachability
...
def mesh_health() -> dict[str, Any]:
    ...
    A pure delegate to :func:`~mesh.core.search.search_health` — the exact
    call ``mesh search --health`` already makes, so the two surfaces read
    ...
    Distinct from the withheld ``mesh_status``: this reports recall-path
...
def mesh_session_start(
    ...
    The MCP mirror of ``mesh session-start`` (team-awareness/10) — the highest-
    ...
    does — ``task_list(mine=True)``, :func:`~mesh.core.lenses.session_mentions`
    (inbound links to nodes the caller owns or has claimed), and
    ``recent_activity(since=7d)`` — via the shared
    :func:`~mesh.core.lenses.session_start_entries` composer, so the ordering
    ...
    :func:`~mesh.core.lenses.as_effective_agent`) — "what would that agent's
...
def mesh_note_new(
    ...
    # either way — see mesh.core.notes.find_duplicate_title.
...
def mesh_task_new(
    ...
    config = load_config()
    # Checked before the write, so a hit names the *prior* id (R9); non-blocking
    # either way — see mesh.core.tasks.find_duplicate_title.
...
def mesh_task_release(
    ...
    ``[core].agent`` exactly like :func:`mesh_task_claim`'s ``claimer``.
...
def mesh_task_update(
    ...
    ``mesh_task_claim``/``mesh_task_release`` for the execution handle.
    """
    config = load_config()
    task = update_task(
        config,
...
_KIND_BY_TYPE: tuple[tuple[type[MeshError], str], ...] = (
...
# Fallback for a MeshError not named above (e.g. a future subclass): derive a
...
def _error_kind(exc: MeshError) -> str:
...
def _guarded(fn: Callable[..., Any]) -> Callable[..., Any]:
    ...
    (``mesh.cli._errors.cli_errors``) — ``MeshError`` (``LockError`` and
    ...
    tool error naming ``mesh init``.

    The module-level ``mesh_*`` functions are left unwrapped so they stay
...
def _register() -> None:
    """Register every ``mesh_*`` tool with its annotation; called once at import."""
    # Read-only.
    app.tool(_guarded(mesh_note_get), annotations=_READ_ONLY)
    app.tool(_guarded(mesh_note_list), annotations=_READ_ONLY)
    app.tool(_guarded(mesh_task_get), annotations=_READ_ONLY)
    app.tool(_guarded(mesh_task_list), annotations=_READ_ONLY)
    app.tool(_guarded(mesh_search), annotations=_READ_ONLY)
    app.tool(_guarded(mesh_health), annotations=_READ_ONLY)
    app.tool(_guarded(mesh_recent_activity), annotations=_READ_ONLY)
    app.tool(_guarded(mesh_build_context), annotations=_READ_ONLY)
    app.tool(_guarded(mesh_graph), annotations=_READ_ONLY)
    app.tool(_guarded(mesh_project), annotations=_READ_ONLY)
    app.tool(_guarded(mesh_session_start), annotations=_READ_ONLY)
    # Write (no special hint).
    app.tool(_guarded(mesh_note_new))
    app.tool(_guarded(mesh_note_append))
    app.tool(_guarded(mesh_task_new))
    app.tool(_guarded(mesh_task_append))
    # Idempotent.
    app.tool(_guarded(mesh_note_update), annotations=_IDEMPOTENT)
    app.tool(_guarded(mesh_task_claim), annotations=_IDEMPOTENT)
    app.tool(_guarded(mesh_task_release), annotations=_IDEMPOTENT)
    app.tool(_guarded(mesh_task_finish), annotations=_IDEMPOTENT)
    app.tool(_guarded(mesh_task_update), annotations=_IDEMPOTENT)
    # Destructive.
    app.tool(_guarded(mesh_task_cancel), annotations=_DESTRUCTIVE)
...
def main() -> None:  # pragma: no cover - process entry point
    """Serve the mesh MCP tools (stdio transport). Wired as ``python -m mesh.mcp.server``."""
    app.run()
