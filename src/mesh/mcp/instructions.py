...
A pure function of :class:`~mesh.schemas.config.Config` (or its absence): no I/O,
...
``FastMCP("mesh", instructions=...)``. Every MCP client (Cowork included, which
...
what mesh is, identity, valid owners, vault, recall mode, the tag-mutation trap,
...
_WHAT_MESH_IS = (
    ...
    "Markdown is the source of truth; mesh owns writes and fast reads — no "
...
def _identity_section(config: Config | None) -> str:
    if config is None:
        return (
            "## Your identity\n"
            "No config could be loaded — run `mesh init`, then restart this MCP "
    ...
    if not config.agent:
        return (
            "## Your identity\n"
            "No agent identity configured ([core].agent / $MESH_AGENT unset) — run "
            "`mesh init`, or pass an explicit claimer/owner to calls that need one "
            "(task_claim, task_release, note/task creation)."
        )
    return (
        "## Your identity\n"
        f"You are `{config.agent}` this session (from [core].agent / "
        "$MESH_AGENT) — tools with a claimer/owner param default to it when "
        "omitted."
    )
...
def _recall_section(config: Config | None) -> str:
    ...
    return (
        ...
        "unreachable — call mesh_health, or check a hit's mode field, "
...
def build_instructions(config: Config | None) -> str:
    ...
    "run ``mesh init``" degradation instead of guessing. Pure: no I/O, and the
