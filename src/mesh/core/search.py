...
``search`` is a recall verb with two entry points (the ``mesh search`` CLI
command and the ``mesh_search`` MCP tool) that must behave identically. This
...
* :func:`hit_dict` — render one :class:`~mesh.schemas.search.SearchResult` to the
...
def resolve_effective_threshold(flag: float | None, config: Config) -> float | None:
    ...
    silently-defaulted cutoff. Shared by the ``mesh search`` CLI command and
    the ``mesh_search`` MCP tool so this three-way resolution is one mechanism,
...
def search_health(config: Config) -> dict[str, Any]:
    ...
    executable is even on ``PATH``, via :func:`~mesh.index.indexed_client.indexed_available`)
...
def _iso_z(value: datetime) -> str:
    ...
    Mirrors :func:`mesh.schemas.note._iso_z` — kept as a local one-liner per
