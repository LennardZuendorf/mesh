...
:func:`mesh.core.wikilinks.resolve_wikilinks` (an on-disk scan, daemon-free):
...
from mesh.core.ids import generate_note_id
...
from mesh.storage.files import atomic_write, dump_post, iter_md, note_folder, read_post
...
def _iter_note_files(config: Config) -> Iterator[Path]:
    ...
    shared vault walk (:func:`mesh.storage.files.iter_md`).
    """
...
def _resolve_path(config: Config, id_or_slug: str) -> Path:
    """Resolve ``<id|slug>`` to a *mesh* note path, sandbox-checked.

    Only files whose stem carries a mesh id (``n-`` prefix) are candidates, so a
    ...
    mesh_files = [p for p in _iter_note_files(config) if p.stem.startswith(_ID_PREFIX)]
    by_id = [p for p in mesh_files if p.stem == id_or_slug]
    if by_id:
...
def _format_stamp(iso: str, agent: str | None) -> str:
    ...
    ``append``) and :func:`mesh.core.tasks._terminate_task` (``## Outcome`` /
    ...
    When ``agent`` is unset (no ``[core].agent``, no ``$MESH_AGENT``) this
...
def create_note(
    ...
    *,
    ...
    defaults to the resolved config agent (``$MESH_AGENT`` override applied at
    ...
    ``notes/.locks/_create.lock`` (see :func:`mesh.storage.locks.allocator_lock_path`),
...
def _validated_note(meta: dict[str, Any], target: str) -> Note:
    ...
    A file whose frontmatter does not satisfy the schema is not a note mesh can
...
def append_note(
    ...
    *,
    ...
    the lock goes through :func:`mesh.storage.files.read_post`; a file that
...
def apply_tag_spec(existing: list[str], spec: str) -> list[str]:
    ...
    an error (CLI exit 2 via :func:`mesh.cli._errors.cli_errors`, an MCP
...
def update_note(
    ...
    *,
    ...
    :func:`mesh.storage.files.read_post`; a file that vanishes or turns
...
def delete_note(config: Config, id_or_slug: str) -> str:
    ...
    delete. :func:`mesh.storage.locks.hold` clears only *stale* locks (dead PID
...
def note_rows(config: Config) -> Iterator[MetaRow]:
    ...
    :func:`mesh.storage.files.read_post`, so a foreign or corrupt sibling never
    crashes the walk.
    """
...
def _title_collision(rows: Iterable[MetaRow], title: str, id_prefix: str) -> str | None:
    ...
    :func:`mesh.core.tasks.find_duplicate_title`: each passes its own row
...
def find_duplicate_title(config: Config, title: str) -> str | None:
    ...
    :func:`mesh.core.wikilinks._title_index` uses for wikilink title
    ...
    title-identical one (see :func:`mesh.core.tasks.find_duplicate_title` for
    the task-side twin).
...
def select_notes(rows: Iterable[MetaRow], spec: NoteFilter) -> list[NoteView]:
    ...
    whose frontmatter carries a valid mesh id (``n-`` prefix) and validates
...
def list_notes(
    ...
    *,
    ...
    """List mesh notes under ``notes/``, filtered and sorted — the on-disk path.
    ...
    :meth:`DaemonClient.note_list <mesh.daemon.client.DaemonClient.note_list>`.
    """
    return select_notes(
        note_rows(config),
        NoteFilter.build(
