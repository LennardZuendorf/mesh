"""Read-lens layer — the one composable home for mesh's read-only views.
...
* **graph-query** — :func:`graph_query` (re-exported from
  ...
  (:func:`~mesh.core.context.inbound_ids`, the backlink/notify view), or
  their union.
...
* **vault-status** — :func:`status_inputs` + :func:`status_report`: the
  vault-health composite behind ``mesh status``. The daemon's ``vault.status``
...
from mesh.core.notes import NoteError, NoteView, _parse_since, get_note
from mesh.core.tasks import TaskView
from mesh.core.wikilinks import find_dangling
from mesh.daemon.client import DaemonClient
from mesh.schemas.config import Config
from mesh.storage.files import read_body
...
from mesh.storage.locks import _is_stale
...
# ``mesh_session_start`` mirror cannot drift apart on the window itself.
...
def session_mentions(
    ...
    *,
    ...
    (:func:`mesh.core.context._inbound_index`) is built **once** — not once per
    ...
    :func:`mesh.core.context._resolve_entry` — the same single-node resolver
    :func:`~mesh.core.context.build_context`/:func:`~mesh.core.context.graph_query`
    ...
    recency floor (the identical window :func:`mesh.core.activity.recent_activity`
    applies, parsed once via :func:`mesh.core.notes._parse_since`, and reusing
...
def scan_stale_locks(config: Config) -> list[Path]:
    ...
    exact rule enforced on acquire (:func:`mesh.storage.locks._is_stale`), reused
    here so ``mesh status`` and the locker never disagree. Lock files are not
...
def status_inputs(notes: list[NoteView], tasks: list[TaskView]) -> dict[str, Any]:
    ...
    behind one ``mesh status``. Both callers reduce their views through this one
...
def status_report(
    ...
    *,
    ...
    a one-row recency window (:meth:`~mesh.index.warm.VaultIndex.recent` or
    :func:`~mesh.index.warm.scan_recent`) whose ``mtime`` drives freshness.
    ...
    lock-directory listing rather than a parse. So ``mesh status`` is warm in its
...
class ProjectNotFoundError(MeshError):
...
def project_view(config: Config, project_id: str) -> ProjectResult:
    ...
    resolution :func:`mesh.core.notes.get_note` performs) and pairs it with
    :func:`mesh.core.tasks.list_tasks`'s ``project``-filtered result (all
