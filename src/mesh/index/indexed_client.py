...
Mesh owns the *interface* to search, not the ranking: this module shells the
...
* **:func:`search`** — run ``indexed index search "<q>" --collection <c> --json
  ...
  :class:`SearchResult` by reading the frontmatter at ``path``. Foreign (non-mesh)
  ...
  :func:`~mesh.index.fallback.search_fallback`.
...
* **:func:`incremental_update` / :func:`full_rebuild` / :func:`reindex`** — keep the
  ...
  thread and let failures propagate. ``reindex`` is the delegate ``mesh reindex``
  calls. The daemon subscribes :func:`incremental_update` to its own
  :class:`~mesh.index.watcher.ChangeHooks` registry directly (an inlined lambda at
  ``daemon/server.py`` — see :func:`mesh.daemon.server.DaemonServer.start`), rather
...
def indexed_available() -> bool:
    ...
    A cheap, side-effect-free probe for :func:`~mesh.core.search.search_health`:
...
def search(
    ...
    delegates to :func:`~mesh.index.fallback.search_fallback` (forwarding ``quiet``
...
def incremental_update(config: Config, path: Path) -> None:
    ...
    whole lifetime. Incremental freshness is best-effort; ``mesh reindex`` (a full
...
def reindex(config: Config) -> None:
    """Full rebuild — the delegate ``mesh reindex`` calls."""
    full_rebuild(config)
