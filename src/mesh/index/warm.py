...
* **:class:`VaultIndex`** — an in-process frontmatter index of the vault corpus.
  ...
  most-recently-modified *mesh* rows for ``activity.recent``, and
...
* **:func:`scan_recent`** — the daemon-down fallback for ``activity.recent``: an
  mtime-sorted dir scan of ``notes/`` and ``tasks/`` (mesh-id files only),
  returning the same JSON-serializable row shape and ordering as
  :meth:`VaultIndex.recent`.

**Mesh rows vs. the corpus.** ``note list`` / ``task list`` only ever surface
files carrying a mesh id (``n-``/``t-``), but ``search`` (tag-pull and the
...
:mod:`mesh.index.watcher`; folder reconcile lives in
:mod:`mesh.index.reconcile`. Both build on the small shared helpers below.
"""
...
def _is_mesh_id(value: object) -> TypeGuard[str]:
    """Whether ``value`` is a mesh id (``n-``/``t-``) — narrowing, so callers can
...
def iter_vault_md(vault: Path) -> Iterator[Path]:
    ...
    :func:`mesh.index.tagpull.iter_corpus` walks, shared verbatim so the warm
...
class IndexEntry:
    """One indexed vault file: its mesh id (``None`` if foreign), path, mtime, meta."""
...
class VaultIndex:
    ...
    **One row per file, keyed by real path — never by mesh id.** The id is a
    *field* on the row, not its identity, because the folder cannot promise ids are
    ...
    Mesh entities (``n-``/``t-``) and coexisting foreign Markdown share the one
    ...
    :meth:`recent` speak for the mesh rows only; :meth:`corpus` returns every row.
    """

    def __init__(self) -> None:
        self._rows: dict[str, IndexEntry] = {}  # realpath -> row (mesh *and* foreign)
    ...
    def reparse(self, path: Path) -> None:
        ...
        Reads through :func:`mesh.storage.files.read_post` — the project's one
        ...
        A file that gains, loses or *changes* its mesh id needs no bookkeeping:
        ...
        raw_id = meta.get("id")
        entry_id: str | None = raw_id if _is_mesh_id(raw_id) else None
    ...
    def entries(self) -> list[IndexEntry]:
        """Every indexed *mesh* entity (id-bearing), as a snapshot list.
    ...
    def corpus(self) -> list[IndexEntry]:
        """Every indexed vault file — mesh entities *and* foreign ones.
    ...
    def recent(self, limit: int = DEFAULT_RECENT_LIMIT) -> list[dict[str, Any]]:
        """Most-recently-modified mesh rows, mtime-descending (id-ascending on ties)."""
    ...
    def __len__(self) -> int:
        """The number of indexed *mesh* entities (foreign corpus rows excluded)."""
...
def scan_recent(config: Config, limit: int = DEFAULT_RECENT_LIMIT) -> list[dict[str, Any]]:
    """Mtime-sorted dir scan of ``notes/``/``tasks/`` (mesh-id files only).
