...
gate on a valid shards id). Coexisting non-shards (foreign) Markdown is included;
...
:mod:`~shards.index.fallback` reuses the exact same corpus and parsing.
"""
...
def read_row(path: Path) -> CorpusRow | None:
    ...
    Thin adapter over :func:`shards.storage.files.read_post` — the single safe
...
def shards_id(meta: dict[str, Any]) -> str | None:
    """The frontmatter ``id`` iff it is a shards id (``n-``/``t-``), else ``None``."""
    raw = meta.get("id")
