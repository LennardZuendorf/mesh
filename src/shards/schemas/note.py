...
The two shards schemas that carry foreign keys — :class:`Note` and
:class:`~shards.schemas.task.Task` — inherit this base; the task schema is a note
...
class _Frontmatter(msgspec.Struct, kw_only=True):
    """Base for shards frontmatter schemas: known-field validation + unknown-key round-trip.

    ``extra`` holds every frontmatter key shards does not own, captured on
...
class Note(_Frontmatter, kw_only=True):
    """Frontmatter for a shards-owned note.
