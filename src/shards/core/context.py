...
three-verbs contract — can surface backlinks via ``shards graph <id>
...
from shards.core.notes import NoteError, get_note, note_rows
from shards.core.tasks import TaskError, get_task, task_rows
...
def _inbound_index(config: Config) -> dict[str, list[str]]:
    ...
    Walks the corpus once via :func:`shards.core.notes.note_rows` (``notes/``)
    ...
    (:func:`shards.core.wikilinks.resolve_wikilinks`) — no title lookup of its
    ...
    :func:`shards.core.wikilinks.find_dangling` already pays for ``shards
    ...
    deterministic order — the same reason :func:`shards.core.notes.select_notes`
...
def inbound_ids(config: Config, target_id: str) -> list[str]:
    ...
    pure inverse of the forward edge :func:`shards.core.notes.append_note` /
    :func:`shards.core.tasks.update_task` (and friends) recompute from a body's
