...
class _RootGroup(TyperGroup):
    ...
    keeps ``shards --help`` in the declared verbs → admin → session-lenses order
    (:data:`_ORDER`) instead.
    ...
    *before* that block in every one of them (a bare call at the top of the
    command body) — a :class:`~shards.schemas.config.ConfigMissingError` raised
    ...
    unchanged (it is a ``RuntimeError``, never a ``ShardsError``/``ValueError``/
